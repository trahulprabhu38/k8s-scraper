...
# excel_file = f"/mnt/data/input.csv"
# output_file = f"/mnt/data/result.csv"
...
# Async function to scrape one university
async def main(website_url):
    task = f"""
        You are tasked with exploring the university website provided and extracting the following details:
        The website URL is: {website_url}
        [Full prompt omitted for brevity – keep same as yours]
    """
    agent = Agent(task=task, llm=llm, controller=controller)
    result: ActionResult = await agent.run()
    result_agent = result.final_result()
...
# Async function to process all universities
async def process_universities_from_excel(excel_file, output_file):
    df = pd.read_csv(excel_file)
    headers = [
        "university_name", "programs", "top_programs", "city", "state", "country",
    ...
    for idx, row in df.iterrows():
        website_url = row.get("HD2023.Institution's internet website address", "").strip()
        if not website_url:
            print(f"[{idx}] ⚠️ No URL found, skipping.")
            continue
        try:
            data = await main(website_url)
            if not data:
                print(f"[{idx}] ❌ No data returned from agent for {website_url}")
                continue

            # Flatten result for CSV
            result_row = {
                "university_name": data.get("university_name", ""),
                "programs": safe_join(data.get("programs")),
                "top_programs": safe_join(data.get("top_programs")),
                "city": data.get("city", ""),
                "state": data.get("state", ""),
                "country": data.get("country", ""),
                "degree_levels": safe_join(data.get("degree_levels")),
                "mode_of_study": safe_join(data.get("mode_of_study")),
                "qs_ranking": data.get("qs_ranking", ""),
                "acceptance_rate": data.get("acceptance_rate", ""),
                "institution_type": data.get("institution_type", ""),
                "campus_facilities": safe_join(data.get("campus_facilities")),
                "total_students": data.get("total_students", ""),
                "residential_students": data.get("residential_students", ""),
                "non_residential_students": data.get("non_residential_students", ""),
                "about_university": data.get("about_university", ""),
                "admission_requirements": safe_join(data.get("admission_requirements")),
                "scholarships": safe_join(data.get("scholarships")),
                "cost_info": safe_join(data.get("cost_info")),
                "tuition_fee_structure": safe_join(data.get("tuition_fee_structure")),
                "application_fee": data.get("application_fee", ""),
                "housing_fee_info": safe_join(data.get("housing_fee_info")),
                "living_expenses_per_month": data.get("living_expenses_per_month", ""),
                "total_cost_of_attendance": data.get("total_cost_of_attendance", ""),
                "student_experience_details": data.get("student_experience_details", ""),
                "campus_life_highlights": data.get("campus_life_highlights", ""),
                "university_size": data.get("university_size", "")
            }

            # Append to CSV after each success
            pd.DataFrame([result_row]).to_csv(output_file, mode='a', header=not os.path.exists(output_file), index=False)
            print(f"[{idx}] ✅ Saved data for {website_url}")

        except Exception as e:
            print(f"[{idx}] ❌ Error scraping {website_url}: {e!r}")
